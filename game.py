...
class Game(object):
    def __init__(self):
        ...
        # Create the player
        #At first we make a normal player
        self.normal = True
        self.player = Player(32,128,"player.png",'walk.png')
    ...
    def run_logic(self):
        if not self.game_over:
            if self.score == 10 and self.normal:
                self.normal = False
                self.player = Player(self.player.rect.x,self.player.rect.y, "super.png","super-walk.png")
                #self.player = Player(32,128,'super.png','walk.png')
                self.score = 0
                self.start_ticks=pygame.time.get_ticks() #starter tick
            
            
            if not self.normal and (pygame.time.get_ticks()-self.start_ticks)/1000 > 10: # if more than 10 seconds go back to normal pacman
                self.normal = True
                self.player = Player(self.player.rect.x, self.player.rect.y, "player.png", "walk.png")
                self.score = 0
            ...
            if len(block_hit_list) > 0:
                ...
                self.score += 1
                self.eaten += 1
            #check if pacman has collided with enemies
            block_hit_list = pygame.sprite.spritecollide(self.player,self.enemies,True)
            if len(block_hit_list) > 0:
                #if this is normal pacman, pacman explodes and game is over
                if self.normal:
                    self.player.explosion = True
                    #self.game_over_sound.play()
                #if it is super pacman
                elif not self.normal:
                    #super pacman has eaten a ghost
                    #self.pacman_sound.play()
                    #now we need to find out which ghost should be deleted
                    for enemy in self.enemies:
                        if (enemy.rect.x -self.player.rect.x)**2 + (enemy.rect.y -self.player.rect.y)**2 < 10:
                            self.enemies.remove(enemy)
            self.game_over = self.player.game_over
            self.enemies.update(self.horizontal_blocks,self.vertical_blocks)
            #if all the food is eaten
            if len(self.dots_group) == 0:
                print("Won! You ate all the food")
                screen = pygame.display.set_mode((SCREEN_WIDTH,SCREEN_HEIGHT))
                screen.fill(BLACK)
                txt = self.font.render("You won! You ate all the food", True, WHITE)
                width = txt.get_width()
                ...
                self.game_over = True
            #if all the ghosts are eaten
            if len(self.enemies) ==0:
                print("Won! You ate all the ghosts!")
                screen = pygame.display.set_mode((SCREEN_WIDTH,SCREEN_HEIGHT))
                screen.fill(BLACK)
                txt = self.font.render("You won! You ate all the ghosts", True, WHITE)
                width = txt.get_width()
                height = txt.get_height()
                # Determine the position of the label
                posX = (SCREEN_WIDTH /2) - (width /2)
    ...
    def display_frame(self,screen):
        ...
        if self.game_over:
            if self.about:
                self.display_message(screen,"Pacman by PDA")
